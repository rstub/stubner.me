...
On Stack Overflow the [question](https://stackoverflow.com/q/56832560/8416610) was asked how to calculate the integral

$$
\int_{-\infty}^{\infty} \mathrm{d}x \exp\left(-\frac{(x-\mu)^4}{2}\right)
$$

in Rcpp, e.g. by using [RcppNumerical](https://github.com/yixuan/RcppNumerical). In principle this should be straight forward. One defines a class that extends `Numer::Func` for the function and an interface function that calls `Numer::integrate` on it:
...
But produces `NA` for infinte ones:
...
This is disappointing, since base R's `integrate()` handles this case without problems:
...
Second, since the function is almost zero almost everywhere, it is suffcient to integrate over a small region around `mean` to get a reasonable approviamtion for the integral over the infinite range:

```{r integrant_plot, echo=FALSE}
par(mar = c(5, 5, 4, 2) + 0.1)
plot(function(x) exp4(x, 4), 0, 8, ylab = expression(exp(-(x-mu)^4/2)), main = "Integrant for mean = 4")
```
```{r}
...
This is infact done by a different function from R's C-API: `Rdqagi()` instead of `Rdqags()`. In principle one could call `Rdqagi()` via Rcpp, but this is not straightforward. Fortunately, there are at least two other solutions.
...
Alternatively, one can apply the the transformation used by GSL (and probably R) also in conjunction with RcppNumerical. To do so, one has to substitute $x = (1-t)/t$ resulting in
