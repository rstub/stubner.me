...
was given. In principle this should be straight forward. One defines a class that extends `Numer::Func` for the function and an interface function that calls `Numer::integrate` on it:
...
But produces `NA` for infinite ones:
...
Second, the function is almost zero almost everywhere:
